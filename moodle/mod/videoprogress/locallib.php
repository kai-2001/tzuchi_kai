...
/** 進度容錯門檻：超過此百分比自動視為 100% */
define('VIDEOPROGRESS_TOLERANCE_THRESHOLD', 95);
...
/** 最大單次儲存片段長度（秒） */
define('VIDEOPROGRESS_MAX_SEGMENT_LENGTH', 30);

/** 壓縮失敗最大重試次數 */
define('VIDEOPROGRESS_MAX_RETRY_ATTEMPTS', 3);

/** 壓縮項目視為卡住的超時時間（秒） */
define('VIDEOPROGRESS_STUCK_TIMEOUT', 600);

/** 壓縮失敗重試間隔（秒） */
define('VIDEOPROGRESS_RETRY_INTERVAL', 300);

/**
 * 驗證 URL 是否安全（防止 SSRF 攻擊）
 * 
 * 檢查 URL 是否指向本機位址，拒絕可能的 SSRF 攻擊
 * 注意：允許內網 IP (10.x.x.x, 192.168.x.x) 以支援學校內部 CMS
 *
 * @param string $url 要驗證的 URL
 * @return bool true 表示安全，false 表示不安全
 */
function videoprogress_is_safe_url($url)
{
    if (empty($url)) {
        return false;
    }
    ...
    // [改進] 允許內網 IP (10.x.x.x, 192.168.x.x 等) 以支援學校內部 CMS
    // 只封鎖 localhost (127.x.x.x) 和雲端 metadata (169.254.x.x)
    if (strpos($ip, '127.') === 0) {
        return false;
    }

    // 封鎖 AWS/雲端 metadata 端點 (169.254.x.x)
...
function videoprogress_update_progress($videogressid, $userid, $lastposition = null)
{
    ...
    if ($videoprogress->videotype === 'external' && $duration <= 0) {
        ...
        // [Fix] 容錯處理：如果進度超過 95%，直接視為 100%（解決 98% 未完成問題）
...
function videoprogress_detect_external_video($externalurl)
{
    ...
    // [Security] SSRF 防護：驗證 URL 是否安全
    if (!videoprogress_is_safe_url($externalurl)) {
        debugging('videoprogress: Blocked unsafe URL: ' . $externalurl, DEBUG_DEVELOPER);
        return null;
    }
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_MAXREDIRS => 5,
        CURLOPT_TIMEOUT => 10,
        CURLOPT_SSL_VERIFYPEER => false, // [Note] 關閉 SSL 驗證以支援內部 CMS 的自簽憑證
