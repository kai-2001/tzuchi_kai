...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'compress') {
    ...
    try {
        ...
        if (!$item) {
            die(json_encode(['success' => false, 'error' => get_string('error:queue_not_found', 'videoprogress')]));
        }
        set_time_limit(600);
...
if ($videoprogress->videotype === 'external' && !empty($videoprogress->externalurl)) {
    ...
    if (!empty($result['duration']) && $videoprogress->videoduration != $result['duration']) {
...
switch ($videoprogress->videotype) {
    ...
    case 'external':
        if ($videoData['use_html5'] ?? false) {
            $infoText = ($videoData['is_evercam'] ?? false) ? get_string('mode:evercam', 'videoprogress') : get_string('mode:video_detected', 'videoprogress');
    ...
    case 'upload':
        if ($videoData['is_zip'] ?? false) {
            echo player_renderer::html5($videoData['url'], $videoData['chapters'], $videoData['duration'], get_string('mode:zip_package', 'videoprogress'));
        } else {
