...
class compression_service
{
    ...
    public static function get_pending_items(?int $contextid = null): array
    {
        ...
        // ORDER BY: 
        // 1. status = 'processing' DESC (Processing items first)
        // 2. priority DESC (Highest priority next)
        // 3. timecreated ASC (Time order for same priority)
        $sql .= " ORDER BY CASE WHEN q.status = 'processing' THEN 0 ELSE 1 END, q.priority DESC, q.timecreated ASC";
        ...
        foreach ($items as $item) {
            ...
            $item->activity_url = new \moodle_url('/mod/videoprogress/view.php', ['id' => $item->cmid]);
    ...
    public static function get_completed_logs(int $limit = 20): array
    {
        ...
        foreach ($logs as $log) {
            // [Option C: Auto-cleanup]
            if (empty($log->cmid)) {
                $DB->delete_records('videoprogress_compression_log', ['id' => $log->id]);
                continue;
            }

            $log->course_name = $log->course_name ?? '未知';
            $log->activity_name = $log->activity_name ?? '未知';
            $log->activity_url = new \moodle_url('/mod/videoprogress/view.php', ['id' => $log->cmid]);
