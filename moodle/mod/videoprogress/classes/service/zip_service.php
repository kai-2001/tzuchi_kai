...
class zip_service
{
    ...
    public static function process_package(int $contextid, \stored_file $zipfile)
    {
        ...
        if (!$result) {
            return get_string('zip_extract_failed', 'mod_videoprogress');
        }
        ...
        if (!$indexfile) {
            return get_string('zip_no_index', 'mod_videoprogress');
        }
    ...
    private static function validate(\stored_file $zipfile)
    {
        ...
        if (!$files) {
            return get_string('zip_invalid', 'mod_videoprogress');
        }

        // 檢查檔案數量
        if (count($files) > self::MAX_FILES) {
            return get_string('zip_too_many_files', 'mod_videoprogress', self::MAX_FILES);
        }
        ...
        foreach ($files as $file) {
            ...
            if (strpos($file->pathname, '..') !== false) {
                return get_string('zip_path_traversal', 'mod_videoprogress');
            }
            ...
            if (!$file->is_directory) {
                ...
                if (!in_array($ext, self::ALLOWED_EXTENSIONS)) {
                    return get_string('zip_bad_extension', 'mod_videoprogress', $ext);
                }
                $totalSize += $file->size;
            }
        }
        ...
        if ($totalSize > self::MAX_UNCOMPRESSED_SIZE) {
            return get_string('zip_too_large', 'mod_videoprogress');
        }
