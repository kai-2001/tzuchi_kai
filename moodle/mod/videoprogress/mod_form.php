...
class mod_videoprogress_mod_form extends moodleform_mod {
    ...
    public function validation($data, $files) {
        ...
        if ($data['videotype'] === 'external') {
            if (empty($data['externalurl'])) {
                ...
                if (!videoprogress_is_safe_url($data['externalurl'])) {
                    $errors['externalurl'] = get_string('error:unsafe_url', 'videoprogress');
                }
            }
        }
        ...
        if ($data['videotype'] === 'upload' && !empty($data['videofile'])) {
            ...
            if ($draftfile && strtolower(pathinfo($draftfile->get_filename(), PATHINFO_EXTENSION)) === 'zip') {
                ...
                if (!$hasIndexHtml) {
                    $errors['videofile'] = get_string('error:zip_no_index', 'videoprogress');
                }
            }
        }
    ...
    public function definition_after_data() {
        ...
        if ($mform->elementExists('completionview')) {
            $mform->removeElement('completionview');
        }
        ...
        // [UI Fix] 透過 AMD 模組修改標準表單的文字
        // 將 "新增條件" 改為 "依據完成門檻判斷完成"
        global $PAGE;
        $PAGE->requires->js_call_amd('mod_videoprogress/form_completion_fix', 'init');
    }
