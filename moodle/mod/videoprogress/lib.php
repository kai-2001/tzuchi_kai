...
function videoprogress_add_instance($data, $mform = null)
{
    ...
    if ($data->videotype === 'external' && !empty($data->externalurl)) {
        ...
        if (!videoprogress_is_safe_url($data->externalurl)) {
            throw new \moodle_exception('error:unsafe_url', 'mod_videoprogress');
        }
    }
...
function videoprogress_delete_instance($id)
{
    ...
    // [Complete Cleanup] 徹底清除該活動的所有關聯資料，包括壓縮任務和暫存檔
    // 1. 取得 cmid 以獲取 context
    if ($cm = get_coursemodule_from_instance('videoprogress', $id)) {
        $context = context_module::instance($cm->id);

        // 2. 檢查是否有壓縮佇列
        if ($DB->get_manager()->table_exists('videoprogress_compress_queue')) {
            $queueItems = $DB->get_records('videoprogress_compress_queue');
            $fs = get_file_storage();
            ...
            foreach ($queueItems as $item) {
                try {
                    ...
                    if ($file && $file->get_contextid() == $context->id) {

                        // A. 殺掉 FFmpeg 進程 (Windows & Linux)
                        ...
                        foreach ($tasks as $task) {
                            ...
                            if ($messagedata && isset($messagedata->fileid) && $messagedata->fileid == $item->fileid) {
                                $DB->delete_records('task_adhoc', ['id' => $task->id]);
                            }
                        }

                        // C. [關鍵] 刪除硬碟上的暫存壓縮檔 (_tmp_xxxxx.mp4)
                        global $CFG;
                        $tempdir = $CFG->tempdir . '/videoprogress';
                        // 雖然無法精確得知檔名，但殺掉行程後，系統 temp 機制通常會隨時間清理
                        // 這裡可以嘗試清空該目錄下明顯過期的檔案，但為了避免誤刪其他活動進行中的檔案，
                        // 最安全的做法是依靠殺行程 + Moodle 自帶的 temp 清理機制。
                        // (註：直接 rm -rf 整個目錄太危險，不建議)

                        // D. 刪除 Queue 資料庫紀錄
                        $DB->delete_records('videoprogress_compress_queue', ['id' => $item->id]);
                    }
                } catch (\Exception $e) {
                    continue;
                }
        ...
        if ($DB->get_manager()->table_exists('videoprogress_compression_log')) {
            $DB->delete_records('videoprogress_compression_log', ['contextid' => $context->id]);
        }
    }
...
function videoprogress_save_video_file($data, $mform)
{
    ...
    // 排程壓縮（非 ZIP 情況）
    if (!empty($files)) {
        // [Fix] 清除舊的 package 區域（避免 ZIP 換成 MP4 時偵測錯誤）
        $fs->delete_area_files($context->id, 'mod_videoprogress', 'package');
...
function videoprogress_queue_compression($contextid, $filearea)
{
    ...
    // [Fix] 清除此 Context 下已不存在檔案的舊佇列項目 (避免重複/幽靈任務)
    $dbman = $DB->get_manager();
    if ($dbman->table_exists(new xmldb_table('videoprogress_compress_queue'))) {
        $valid_file_ids = [];
        foreach ($files as $f) {
            $valid_file_ids[] = $f->get_id();
        }

        $queued_items = $DB->get_records('videoprogress_compress_queue', ['contextid' => $contextid]);
        foreach ($queued_items as $item) {
            if (!in_array($item->fileid, $valid_file_ids)) {
                $DB->delete_records('videoprogress_compress_queue', ['id' => $item->id]);
            }
        }
    }
...
function videoprogress_get_completion_state($course, $cm, $userid, $type)
{
    ...
    $progress = $DB->get_record('videoprogress_progress', ['videoprogress' => $videoprogress->id, 'userid' => $userid]);
