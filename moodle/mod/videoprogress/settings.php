...
if ($ADMIN->fulltree) {
    ...
    $manageLink = '<a href="' . $manageUrl . '" class="btn btn-primary btn-sm" style="color: #fff !important;"><i class="fa fa-cogs"></i> ' . get_string('open_compression_management', 'mod_videoprogress') . '</a>';
    ...
    if (PHP_OS_FAMILY !== 'Windows') {
        // 自動佇列處理開關
        $settings->add(new admin_setting_configcheckbox(
            'mod_videoprogress/autoqueue',
            get_string('autoqueue', 'mod_videoprogress'),
            get_string('autoqueue_desc', 'mod_videoprogress'),
            1  // 預設開啟
        ));
    ...
    // ==========================================
    $settings->add(new admin_setting_heading(
        'mod_videoprogress/security_header',
        get_string('security_settings', 'mod_videoprogress'),
        get_string('security_settings_desc', 'mod_videoprogress')
    ));

    // iframe sandbox 放寬設定
    $settings->add(new admin_setting_configcheckbox(
        'mod_videoprogress/iframe_relaxed_sandbox',
        get_string('iframe_relaxed_sandbox', 'mod_videoprogress'),
        get_string('iframe_relaxed_sandbox_desc', 'mod_videoprogress'),
        0  // 預設關閉
    ));
}
