...
function xmldb_videoprogress_upgrade($oldversion)
{
    ...
    // 版本 2026010700 - 模組化架構、BS4 相容 (無資料庫變更)
    if ($oldversion < 2026010700) {
        upgrade_mod_savepoint(true, 2026010700, 'videoprogress');
    }

    // 版本 2026010701-2026010704 - 修復版本號 (無資料庫變更)
    if ($oldversion < 2026010704) {
        upgrade_mod_savepoint(true, 2026010704, 'videoprogress');
    }

    if ($oldversion < 2026011800) {
        // 新增 priority 欄位
        $table = new xmldb_table('videoprogress_compress_queue');
        $field = new xmldb_field('priority', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'last_error');

        if ($dbman->table_exists($table) && !$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        upgrade_mod_savepoint(true, 2026011800, 'videoprogress');
    }
