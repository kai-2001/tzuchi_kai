...
// Site features.
$temp = new admin_settingpage('theme_academi_clean_sitefeatures', get_string('sitefeatures', 'theme_academi_clean'));

// Enable or disable option for the site features block.
$name = 'theme_academi_clean/sitefblockstatus';
...
$name = 'theme_academi_clean/sitefeaturetitle';
...
$name = 'theme_academi_clean/sitefeaturedesc';
...
$name = 'theme_academi_clean/numberofsitefeature';
$title = get_string('numberofsitef', 'theme_academi_clean');
$description = get_string('numberofsitef_desc', 'theme_academi_clean');
$default = 4;
$choices = array_combine( range(1, 4), range(1, 4) );
$temp->add(new admin_setting_configselect($name, $title, $description, $default, $choices));

$sitefeatures = get_config('theme_academi_clean', 'numberofsitefeature');
for ($i = 1; $i <= $sitefeatures; $i++) {

    // Site feature heading.
    $name = 'theme_academi_clean_sitefblock'.$i.'heading';
    $heading = get_string('sitefblock', 'theme_academi_clean', ['block' => $i]);
    $information = '';
    $setting = new admin_setting_heading($name, $heading, $information);
    $temp->add($setting);

    // Site feature enable/disable option.
    $name = 'theme_academi_clean/sitefblock'.$i.'status';
    $title = get_string('status', 'theme_academi_clean');
    $description = get_string('statusdesc', 'theme_academi_clean');
    $default = YES;
    $setting = new admin_setting_configcheckbox($name, $title, $description, $default);
    ...
    // Site feature title.
    $name = 'theme_academi_clean/sitefblock'.$i.'title';
    $title = get_string('title', 'theme_academi_clean');
    $description = get_string('titledesc', 'theme_academi_clean');
    ...
    $name = 'theme_academi_clean/sitefblock'.$i.'content';
    ...
    // Site feature icon.
    $name = 'theme_academi_clean/sitefblock'.$i.'icon';
    $title = get_string('icon', 'theme_academi_clean');
    $description = get_string('icondesc', 'theme_academi_clean');
    $default = 'lang:sitefblockicon'.$i.'_default';
    $setting = new admin_setting_configtext($name, $title, $description, $default);
    $temp->add($setting);

    // Site feature url.
    $name = 'theme_academi_clean/sitefblock'.$i.'url';
    $title = get_string('url', 'theme_academi_clean');
    $description = get_string('urldesc', 'theme_academi_clean', ['block' => $i]);
