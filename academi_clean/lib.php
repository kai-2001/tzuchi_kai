 ...
 * @return void
 */
function theme_academi_clean_page_init(moodle_page $page) {
    global $CFG;
    $page->requires->js_call_amd('theme_academi_clean/theme', 'init');
}
 ...
 * @return string
 */
function theme_academi_clean_process_css($css, $theme) {
    global $OUTPUT, $CFG;
    $css = theme_academi_clean_pre_css_set_fontwww($css);
    // Set custom CSS.
    $customcss = $theme->settings->customcss;
    $css = theme_academi_clean_set_customcss($css , $customcss);
    return $css;
}
 ...
 * @return string $css
 */
function theme_academi_clean_set_customcss($css, $customcss) {
 ...
 * @return bool
 */
function theme_academi_clean_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    static $theme;
    $bgimgs = ['footerbgimg', 'loginbg', 'mspotmedia'];

    if (empty($theme)) {
        $theme = theme_config::load('academi_clean');
    }
 ...
 * @return string
 */
function theme_academi_clean_pre_css_set_fontwww($css) {
    global $CFG;
 ...
 * @return string
 */
function theme_academi_clean_set_fontwww() {
    global $CFG;
    if (empty($CFG->themewww)) {
        $themewww = $CFG->wwwroot."/theme";
    } else {
 ...
 * @return type|string
 */
function theme_academi_clean_get_logo_url($type = 'header') {
    global $OUTPUT;
    static $theme;
    if (empty($theme)) {
        $theme = theme_config::load('academi_clean');
 ...
 * @return string
 */
function theme_academi_clean_get_setting($setting, $format = true) {
    global $CFG, $PAGE;
    ...
    return (isset($return)) ? theme_academi_clean_lang($return) : '';
}
 ...
 * @return string
 */
function theme_academi_clean_lang($key='') {
    $pos = strpos($key, 'lang:');
    if ($pos !== false) {
        list($l, $k) = explode(":", $key);
        if (get_string_manager()->string_exists($k, 'theme_academi_clean')) {
            $v = get_string($k, 'theme_academi_clean');
            return $v;
        } else {
            return $key;
        }
 ...
 * @return string
 */
function theme_academi_clean_get_main_scss_content($theme) {
    global $CFG;
    ...
    if ($filename == 'default.scss') {
        $scss .= file_get_contents($CFG->dirroot . '/theme/academi_clean/scss/preset/default.scss');
    } else if ($filename == 'eguru') {
        $scss .= file_get_contents($CFG->dirroot . '/theme/academi_clean/scss/preset/eguru.scss');
    } else if ($filename == 'klass') {
        $scss .= file_get_contents($CFG->dirroot . '/theme/academi_clean/scss/preset/klass.scss');
    } else if ($filename == 'enlightlite') {
        $scss .= file_get_contents($CFG->dirroot . '/theme/academi_clean/scss/preset/enlightlite.scss');
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_academi_clean', 'preset', 0, '/', $filename))) {
 ...
 */
function theme_academi_clean_get_pre_scss($theme) {
    $scss = '';
    $helperobj = new theme_academi_clean\helper();
 ...
 * @return string
 */
function theme_academi_clean_get_extra_scss($theme) {
