...
/**
 * 院區管理員 - 統一管理介面
 * templates/tabs/hospital_admin_management.php
 * 
 * Explorer 佈局：左側類別樹 + 右側課程列表
 * 合併原 hospital_admin_categories / courses / enrollment
 */
$rootCatId = $_SESSION['management_category_id'] ?? 0;
?>
<style>
    /* ==========================================
   Explorer 佈局
   ========================================== */
    ...
    .cat-dropdown-menu__item.danger:hover {
        background: #fef2f2;
        color: #dc2626;
    }

    .cat-dropdown-menu__item.danger:hover i {
        color: #dc2626;
    }
...
<div id="section-management" class="page-section" style="max-width: 1400px;">
    <div class="section-header">
        <h2><i class="fas fa-th-large"></i> 課程與類別管理</h2>
        <p class="section-subtitle">管理 <?php echo htmlspecialchars($_SESSION['institution'] ?? ''); ?> 院區的類別結構與課程</p>
    </div>
        ...
        <div class="mgmt-main">
            <div class="mgmt-main__header">
                ...
                <div class="mgmt-main__actions" style="display:flex; gap:6px; align-items:center;">
                    <button
                        onclick="if(!selectedCatId){showToast('請先選擇類別','warning');return;} openCreateCategoryModal()"
                        class="btn-secondary"
                        style="font-size:13px; height:32px; padding:0 14px; display:inline-flex; align-items:center; gap:6px;">
                        <i class="fas fa-folder-plus"></i> 新增子類別
                    </button>
                    <a id="createCourseLink" href="<?php echo $web_root; ?>/index.php?page=course_create"
                    ...
                    <div class="cat-dropdown-wrapper" id="catDropdownWrapper">
                        ...
                        <div class="cat-dropdown-menu" id="catDropdownMenu">
                            <div class="cat-dropdown-menu__header" id="catDropdownLabel">類別操作</div>
                            <button class="cat-dropdown-menu__item" id="menuItemSettings"
                                onclick="closeCatDropdown(); if(selectedCatId) openSettingsModal(selectedCatId, selectedCatName);">
                                <i class="fas fa-cog"></i> 類別設定
                            </button>
                            <button class="cat-dropdown-menu__item" id="menuItemRename"
                                onclick="closeCatDropdown(); if(selectedCatId && selectedCatName) openEditCategoryModal(selectedCatId, selectedCatName);">
                                <i class="fas fa-pen"></i> 重新命名
                            </button>
                            <div class="cat-dropdown-menu__divider" id="menuDividerSettings"></div>
                            <button class="cat-dropdown-menu__item"
                                onclick="closeCatDropdown(); loadCoursesForSelected();">
                                <i class="fas fa-sync-alt"></i> 重新整理
                            </button>
                            <div class="cat-dropdown-menu__divider" id="menuDividerDelete"></div>
                            <button class="cat-dropdown-menu__item danger" id="menuItemDelete"
                                onclick="closeCatDropdown(); if(selectedCatId && selectedCatName) deleteCategory(selectedCatId, selectedCatName);">
                                <i class="fas fa-trash"></i> 刪除此類別
                            </button>
                        </div>
                    </div>
                </div>
...
<div class="modal-overlay" id="settingsModal" style="display:none;">
    <div class="modal-content"
        style="width:800px; max-height:90vh; padding:0; border-radius:16px; overflow:hidden; display:flex; flex-direction:column;">
        <div
            style="padding:16px 24px; background:linear-gradient(135deg,#10b981,#059669); color:white; display:flex; justify-content:space-between; align-items:center;">
            <h3 id="settingsModalTitle" style="margin:0; font-size:16px; font-weight:600;"><i class="fas fa-cog"></i>
                訓練設定</h3>
            <button onclick="closeSettingsModal()"
                style="background:none; border:none; color:white; font-size:20px; cursor:pointer;">&times;</button>
        </div>
        <div style="flex:1; overflow-y:auto; padding:24px;" id="settingsModalBody">
            <div style="text-align:center; padding:40px; color:#9ca3af;"><i class="fas fa-spinner fa-spin"></i> 載入中...
            </div>
        </div>
    </div>
</div>
...
<div class="modal-overlay" id="createCategoryModal" style="display:none;">
    <div class="modal-content" style="width:440px; padding:0;">
        <div
            style="padding:16px 20px; background:linear-gradient(135deg,#f59e0b,#d97706); color:white; border-radius:12px 12px 0 0; display:flex; justify-content:space-between; align-items:center;">
            <h3 style="margin:0; font-size:16px;"><i class="fas fa-folder-plus"></i> 新增子類別</h3>
            <button onclick="closeModal('createCategoryModal')"
                style="background:none; border:none; color:white; font-size:20px; cursor:pointer;">&times;</button>
        </div>
        <div style="padding:20px;">
            <div style="margin-bottom:12px;">
                <label style="font-size:12px; color:#6b7280; display:block; margin-bottom:4px;">父類別</label>
                <div id="createCatParentName" style="font-weight:600; font-size:14px;">-</div>
            </div>
            <div style="margin-bottom:16px;">
                <label style="font-size:12px; color:#6b7280; display:block; margin-bottom:4px;">類別名稱 *</label>
                <input type="text" id="createCatName"
                    style="width:100%; height:36px; padding:0 10px; border:1px solid #d1d5db; border-radius:6px; font-size:14px;"
                    placeholder="請輸入類別名稱">
            </div>
            <div id="cohortOption" style="margin-bottom:16px;">
                <label
                    style="display:flex; align-items:center; gap:10px; cursor:pointer; padding:10px 14px; background:#f0f9ff; border-radius:8px; border:1px solid #bae6fd;">
                    <input type="checkbox" id="createCatCohort" checked
                        style="width:16px; height:16px; accent-color:#3b82f6;">
                    <span style="color:#0369a1; font-weight:500; font-size:13px;"><i class="fas fa-users"
                            style="margin-right:4px;"></i>建立群組並歸入職類</span>
                </label>
            </div>
            <div style="display:flex; gap:8px; justify-content:flex-end;">
                <button class="btn-secondary" onclick="closeModal('createCategoryModal')">取消</button>
                <button class="btn-primary" onclick="submitCreateCategory()"><i class="fas fa-check"></i> 建立</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal-overlay" id="editCategoryModal" style="display:none;">
    <div class="modal-content" style="width:440px; padding:0;">
        <div
            style="padding:16px 20px; background:linear-gradient(135deg,#3b82f6,#2563eb); color:white; border-radius:12px 12px 0 0; display:flex; justify-content:space-between; align-items:center;">
            <h3 style="margin:0; font-size:16px;"><i class="fas fa-pen"></i> 編輯類別</h3>
        ...
        <div style="padding:20px;">
            ...
            <div style="display:flex; gap:8px; justify-content:flex-end;">
                <button class="btn-secondary" onclick="closeModal('editCategoryModal')">取消</button>
                <button class="btn-primary" onclick="submitEditCategory()"><i class="fas fa-save"></i> 儲存</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    // 狀態管理
    // ==========================================
    const ROOT_CAT_ID = <?php echo (int) $rootCatId; ?>;
    let selectedCatId = null;
    let selectedCatName = '';
    let treeData = {};       // catId -> { id, name, coursecount, children: [] }
    ...
    function toggleCatDropdown() {
        ...
        if (label) label.textContent = selectedCatName || '類別操作';

        // 判斷是否為第一層類別（直接在 #categoryTree 下的子項）
        const treeItem = document.querySelector(`#categoryTree > .tree-item[data-id="${selectedCatId}"]`);
        const isFirstLevel = !!treeItem;

        // 第一層不能設定必修類別也不能刪除
        const settingsItem = document.getElementById('menuItemSettings');
        const renameItem = document.getElementById('menuItemRename');
        const deleteItem = document.getElementById('menuItemDelete');
        const deleteDivider = document.getElementById('menuDividerDelete');
        if (settingsItem) settingsItem.style.display = isFirstLevel ? 'none' : '';
        if (renameItem) renameItem.style.display = isFirstLevel ? 'none' : '';
        const settingsDivider = document.getElementById('menuDividerSettings');
        if (settingsDivider) settingsDivider.style.display = isFirstLevel ? 'none' : '';
        if (deleteItem) deleteItem.style.display = isFirstLevel ? 'none' : '';
        if (deleteDivider) deleteDivider.style.display = isFirstLevel ? 'none' : '';
    ...
    async function loadCategoryTree() {
        ...
        try {
            const res = await fetch(`${PortalConfig.webRoot}/api/v2/index.php?route=categories/list_children&parent=${ROOT_CAT_ID}`);
            const data = await res.json();
            if (data.success) {
                treeEl.innerHTML = '';
                const cats = data.data || [];
                if (cats.length === 0) {
                    treeEl.innerHTML = '<div style="text-align:center; padding:30px; color:#94a3b8; font-size:12px;">尚無類別</div>';
                    return;
                }
    ...
    function buildTreeNodeEl(cat, indent) {
        ...
        // 第一層類別（indent===12）僅顯示刪除（需額外確認），不可編輯/設定
        const isFirstLevel = (indent === 12);
        if (!isFirstLevel) {
            actions.innerHTML = `
            <button class="tree-item__action-btn" title="設定" onclick="event.stopPropagation(); openSettingsModal(${cat.id}, '${escapeHtml(cat.name)}')"><i class="fas fa-cog"></i></button>
            <button class="tree-item__action-btn" title="編輯" onclick="event.stopPropagation(); openEditCategoryModal(${cat.id}, '${escapeHtml(cat.name)}')"><i class="fas fa-pen"></i></button>
            <button class="tree-item__action-btn danger" title="刪除" onclick="event.stopPropagation(); deleteCategory(${cat.id}, '${escapeHtml(cat.name)}')"><i class="fas fa-trash"></i></button>
        `;
        }
    ...
    function buildBreadcrumbFromDOM(rowEl) {
        ...
        let html = `<a onclick="onSelectCategory(${ROOT_CAT_ID}, '根目錄')"><i class="fas fa-home"></i></a>`;
        ...
        try {
            ...
            if (Array.isArray(enrolledUsers) && enrolledUsers.length > 0) {
                ...
                for (const u of enrolledUsers.slice(0, 50)) {
                    ...
                    const moodleId = u.moodle_id || u.id; // fallback
                    const portalId = u.portal_id || u.id; // fallback
            ...
            if (ruleSnapshotStr) {
                try {
                    ...
                    if (resolvedGroups.length > 0) {
                        ...
                        // fallback: 沒有 resolved_groups 但有 filter_groups
    ...
    async function removeUserFromCourse(courseId, portalId, moodleId, userName, btnEl) {
        ...
        try {
            ...
            formData.append('user_ids', portalId); // For the unenrol api as fallback if needed
            formData.append('user_id', portalId);  // For visibility remove
            formData.append('moodle_id', moodleId); // Explicitly send Moodle ID
            ...
            if (data.success) {
                if (data.data?.moodle_unenrolled) {
                    ...
                    showToast(`${userName} 的本地記錄已移除，但 Moodle 退選失敗（可能非手動招生方式）。moodleId=${data.data?.moodle_id_used}`, 'warning');
    ...
    // 類別設定 (獨立 Modal)
    // ==========================================
    let currentSettingsCategoryId = null;
    let currentSettingsCategoryName = '';

    function openSettingsModal(categoryId, categoryName) {
        currentSettingsCategoryId = categoryId;
        currentSettingsCategoryName = categoryName;
        document.getElementById('settingsModalTitle').innerHTML = `<i class="fas fa-cog"></i> 訓練設定：${escapeHtml(categoryName)}`;
        document.getElementById('settingsModal').style.display = 'flex';
        document.getElementById('settingsModalBody').innerHTML = '<div style="text-align:center; padding:40px; color:#9ca3af;"><i class="fas fa-spinner fa-spin"></i> 載入中...</div>';
        loadCategorySettings(categoryId);
    }

    function closeSettingsModal() {
        document.getElementById('settingsModal').style.display = 'none';
        currentSettingsCategoryId = null;
        foundMandatoryUsers = [];
    ...
    async function loadCategorySettings(categoryId) {
        try {
            const res = await fetch(`${PortalConfig.webRoot}/api/v2/index.php?route=categories/get_settings&category_id=${categoryId}`);
            const data = await res.json();
            if (data.success) {
                currentSettings = data.settings || {};
                renderSettingsContent(currentSettings);
            } else {
                document.getElementById('settingsModalBody').innerHTML = '<div style="text-align:center; color:#ef4444; padding:40px;">載入設定失敗</div>';
            }
        } catch (e) {
            document.getElementById('settingsModalBody').innerHTML = '<div style="text-align:center; color:#ef4444; padding:40px;">網路錯誤</div>';
        }
    }
    ...
    async function updateSetting(key, value) {
        try {
            ...
            const res = await fetch(PortalConfig.webRoot + '/api/v2/index.php?route=categories/update_settings', {
                method: 'POST', body: formData
            });
            const data = await res.json();
            if (data.success) {
    ...
    function openCreateCategoryModal() {
        document.getElementById('createCatParentName').textContent = selectedCatName || '根目錄';
        document.getElementById('createCatName').value = '';
        document.getElementById('createCategoryModal').style.display = 'flex';
    }

    async function submitCreateCategory() {
        const name = document.getElementById('createCatName').value.trim();
        if (!name) { showToast('請輸入類別名稱', 'warning'); return; }

        const parentId = selectedCatId || ROOT_CAT_ID;
        const formData = new FormData();
        formData.append('name', name);
        formData.append('parent_id', parentId);
        if (document.getElementById('createCatCohort')?.checked) {
            formData.append('create_cohort', 1);
        }

        try {
            const res = await fetch(PortalConfig.webRoot + '/api/v2/index.php?route=categories/create', {
                method: 'POST', body: formData
            });
            const data = await res.json();
            if (data.success) {
                showToast(`類別「${name}」已建立`, 'success');
    ...
    async function submitEditCategory() {
        ...
        try {
            const res = await fetch(PortalConfig.webRoot + '/api/v2/index.php?route=categories/update', {
                method: 'POST', body: formData
            });
            const data = await res.json();
            if (data.success) {
    ...
    async function deleteCategory(catId, catName) {
        if (!confirm(`確定要刪除類別「${catName}」嗎？\n\n⚠️ 警告：這將會刪除該類別下所有課程！`)) return;
        const inputName = prompt(`請輸入類別名稱「${catName}」以確認刪除：\n\n（此操作無法復原）`);
        if (inputName === null) return;
        if (inputName !== catName) { showToast('名稱不符，刪除已取消', 'error'); return; }

        const formData = new FormData();
        formData.append('id', catId);

        try {
            const res = await fetch(PortalConfig.webRoot + '/api/v2/index.php?route=categories/delete', {
                method: 'POST', body: formData
            });
            const data = await res.json();
            if (data.success) {
                showToast(`類別「${catName}」已刪除`, 'success');
                if (selectedCatId == catId) { selectedCatId = null; selectedCatName = ''; }
                loadCategoryTree();
    ...
    function goToEnrol(courseId) {
        let url = `${PortalConfig.webRoot}/index.php?page=course_enrol&course_id=${courseId}`;
    ...
    function goToBatchEnrol() {
        ...
        let url = `${PortalConfig.webRoot}/index.php?page=course_enrol&batch_ids=${ids.join(',')}`;
        if (selectedCatId) url += `&from_cat=${selectedCatId}`;
        window.location.href = url;
    }
    ...
    function renderSettingsContent(s) {
        ...
        <div style="margin-bottom:20px;">
            <label style="display:flex; align-items:center; gap:12px; padding:14px 16px; background:${isMandatory ? '#f0fdf4' : 'white'}; border-radius:10px; border:2px solid ${isMandatory ? '#86efac' : '#e5e7eb'}; cursor:pointer; transition:all 0.2s;">
                <input type="checkbox" id="setMandatory" ${isMandatory ? 'checked' : ''} onchange="toggleMandatorySection()" style="accent-color:#10b981; width:18px; height:18px;">
                <div>
                    <div style="font-weight:600; color:${isMandatory ? '#166534' : '#374151'}; font-size:14px;"><i class="fas fa-star" style="margin-right:4px; color:#f59e0b;"></i>必修類別</div>
                    <div style="font-size:12px; color:${isMandatory ? '#4ade80' : '#9ca3af'};">指定人員必須完成此類別下的課程</div>
                </div>
            </label>
        </div>
        ...
        <!-- 按順序完成（獨立於必修設定之外） -->
        <div style="margin-bottom:20px;">
            <label style="display:flex; align-items:center; gap:12px; padding:14px 16px; background:${isOrdered ? '#fefce8' : 'white'}; border-radius:10px; border:2px solid ${isOrdered ? '#fef08a' : '#e5e7eb'}; cursor:pointer; transition:all 0.2s;">
                <input type="checkbox" id="setOrdered" ${isOrdered ? 'checked' : ''} style="accent-color:#eab308; width:18px; height:18px;">
                <div>
                    <div style="font-weight:600; color:${isOrdered ? '#854d0e' : '#374151'}; font-size:14px;"><i class="fas fa-sort-numeric-down" style="margin-right:4px;"></i>按順序完成</div>
                    <div style="font-size:12px; color:${isOrdered ? '#ca8a04' : '#9ca3af'};">學員須依課程順序依序修課</div>
                </div>
            </label>
        </div>
        
        <!-- 必修詳細設定區 -->
        <div id="mandatoryDetailsSection" style="display:${isMandatory ? 'block' : 'none'}; background:#f8fafc; border-radius:12px; padding:20px; border:1px dashed #cbd5e1; margin-bottom:20px;">
            <h4 style="margin:0 0 16px; font-size:14px; color:#1e293b;"><i class="fas fa-cogs" style="margin-right:6px; color:#6366f1;"></i>必修詳細設定</h4>
            
            ${s.mandatory_user_count > 0 ? `
            <div style="background:linear-gradient(135deg,#dbeafe,#ede9fe); border:1px solid #93c5fd; border-radius:10px; padding:12px 16px; margin-bottom:16px;">
                <div style="display:flex; align-items:center; gap:10px; margin-bottom:${(s.mandatory_users && s.mandatory_users.length > 0) ? '10px' : '0'};">
    ...
    async function saveFullSettings() {
        ...
        try {
            ...
            closeSettingsModal();
            loadCategoryTree();
            // 如果當前選中的類別就是設定的類別，刷新課程
            if (selectedCatId == catId) loadCoursesForSelected();
        } catch (e) {
            ...
            if (selectCatId) {
                try {
                    ...
                    if (apiData.success && catInfo && catInfo.path) {
                        // Moodle path 包含管理根以上的 ID，我們只需要管理根以下的部分
                        let pathIds = catInfo.path.split('/').filter(id => id !== '').map(Number);

                        // 找到 ROOT_CAT_ID 在路徑中的位置，只保留其後面的 ID
                        const rootIndex = pathIds.indexOf(ROOT_CAT_ID);
                        if (rootIndex >= 0) {
                            pathIds = pathIds.slice(rootIndex + 1);
                        }
                        // 確保目標 ID 在路徑末尾
