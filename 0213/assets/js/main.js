...
function redirectWithSSO(targetUrl) {
    ...
    fetch(PortalConfig.webRoot + '/get_sso_url.php?url=' + encodeURIComponent(targetUrl))
...
function showHome() {
    ...
    var adminConsole = document.getElementById('section-admin-console');
    var feats = document.getElementById('section-features');
    if (home) home.classList.add('active');
    if (adminConsole) adminConsole.classList.add('active');
...
function showTab(tabId) {
    ...
    if (targetSection) {
        ...
        if (tabId === 'cohort-management' && typeof window.initCohortManagement === 'function') {
            window.initCohortManagement();
        } else if (tabId === 'course-management' && typeof window.initCourseManagement === 'function') {
            window.initCourseManagement();
        } else if (tabId === 'enrollment-management' && typeof window.loadEnrollmentCourses === 'function') {
            window.loadEnrollmentCourses();
        } else if (tabId === 'category-management' && typeof window.loadCategories === 'function') {
            // Category uses loadCategories globally
            window.loadCategories();
        } else if (tabId === 'users-management' && typeof window.loadMembers === 'function') {
            // Assuming loadMembers is the init for users
            window.loadMembers();
        } else if (tabId === 'tag-management' && typeof window.loadTags === 'function') {
            window.loadTags();
        }
    }
