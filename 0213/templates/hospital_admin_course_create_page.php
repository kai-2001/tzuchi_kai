<?php
/**
 * Portal 課程建立頁面 - 複刻 Moodle course/edit.php 外觀
 */
...
$institution_name = $_SESSION['institution_name'] ?? '院區';
...
try {
    ...
    if (!isset($all_cats['exception'])) {
        // 過濾只顯示院區下的類別
        $allowed_cat_ids = [$mgmt_cat_id => true];
        do {
            $added = false;
...
<div class="page-section">
    ...
    <nav class="breadcrumb-nav">
        ...
        <span>/</span>
        <a href="<?php echo $web_root; ?>/index.php?tab=course-management">課程管理</a>
        <span>/</span>
        ...
        <div class="nav-tabs">
            ...
            <div class="nav-tab-dropdown">
                ...
                <div class="dropdown-menu" id="moreDropdown">
                    <a href="<?php echo $web_root; ?>/index.php?tab=course-management" class="dropdown-item">課程列表</a>
                    <a href="<?php echo $web_root; ?>/index.php?tab=category-management" class="dropdown-item">類別管理</a>
...
<script>
                ...
                switch (submitAction) {
                    case 'filter':
                        ...
                        if (courseId && courseId > 0) {
                            ...
                            setTimeout(() => {
                                window.location.href = PortalConfig.webRoot + '/index.php?page=course_enrol&course_id=' + courseId;
                            }, 1500);
                        } else {
    ...
    async function executeBatchUpload(withEnrol = false) {
        ...
        if (successCount > 0) {
            if (withEnrol && createdCourseIds.length > 0) {
                ...
                window.location.href = `${PortalConfig.webRoot}/index.php?page=course_enrol&course_ids=${courseIdsParam}`;
            } else {
                alert(`成功建立 ${successCount} 個課程${failCount > 0 ? `，${failCount} 個失敗` : ''}`);
                window.location.href = PortalConfig.webRoot + '/index.php?tab=course-management';
            }
            return;
        }
    ...
    function downloadTemplate() {
        event.preventDefault();
        const template = 'fullname,shortname,categoryid,visible,summary\n"範例課程一","SAMPLE001",<?php echo $mgmt_cat_id; ?>,1,"這是範例課程的摘要"\n"範例課程二","SAMPLE002",<?php echo $mgmt_cat_id; ?>,1,"另一個範例課程"';
